...
2. 其内部集成了两个电压比较器，当电压比较器的正向（正极）大于反向（负极）电压时，输出高电平，当电压比较器的正向（正极）小于反向（负极）电压时，输出低电平。而其电压比较器的内部原理图则是这样：
...
3. 将LM393比较器加入电路中，并补全驱动电路和巡线检测电路(`单击图片可放大查看`)：

![](../assets/Triode-Car-Sch4.png)

由于LM393比较器使用的是集电极开路输出，所以需要在输出端加上上拉电阻 R7 与 R8 。
